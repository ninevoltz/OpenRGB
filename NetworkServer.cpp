        ...
        while(this_thread->queue.size() > 0)
        {
            ...
            switch(queue_entry.id)
            {
                case NET_PACKET_ID_RGBCONTROLLER_UPDATELEDS:
                    controllers[this_thread->index]->SetColorDescription((unsigned char *)queue_entry.data);
                    controllers[this_thread->index]->UpdateLEDs();
                    break;
                ...
                case NET_PACKET_ID_RGBCONTROLLER_UPDATEZONELEDS:
                    {
                        ...
                        memcpy(&zone, &queue_entry.data[sizeof(unsigned int)], sizeof(int));

                        controllers[this_thread->index]->SetZoneColorDescription((unsigned char *)queue_entry.data);
                        controllers[this_thread->index]->UpdateZoneLEDs(zone);
                    }
                    break;

                case NET_PACKET_ID_RGBCONTROLLER_UPDATEMODE:
                    controllers[this_thread->index]->SetModeDescription((unsigned char *)queue_entry.data, queue_entry.client_protocol_version);
...
void NetworkServer::SendReply_PluginSpecific(SOCKET client_sock, unsigned int pkt_type, unsigned char* data, unsigned int data_size)
{
    ...
    InitNetPacketHeader(&reply_hdr, 0, NET_PACKET_ID_PLUGINMANAGER_PLUGIN_SPECIFIC, data_size + sizeof(pkt_type));
